...
class PosSession(models.Model):
    ...
    def _get_split_receivable_vals(self, payment, amount, amount_converted):
        ...
        if payment.payment_method_id.allow_for_gift_card:
            partial_vals.update({
                'account_id': self.env.company.gift_card_account_id.id,
            })
    ...
    def _get_combine_receivable_vals(self, payment_method, amount, amount_converted):
        ...
        if payment_method.allow_for_gift_card:
            partial_vals.update({
                'account_id': self.env.company.gift_card_account_id.id,
            })
        return self._debit_amounts(partial_vals, amount, amount_converted)
    ...
    def _prepare_line(self, order_line):
        ...
        if self.config_id.enable_gift_card and (order_line.product_id.id == self.config_id.gift_card_product_id.id):
            res.update({
                'income_account_id': self.env.company.gift_card_account_id.id,
            })
        return res
